      ...
      <section className="section-padding bg-white">
        <div className="container-custom max-w-4xl">
          ...
          <div className="prose prose-lg max-w-none">
            <p className="text-primary-700 leading-relaxed mb-4 text-sm sm:text-base">
      ...
      <section className="section-padding bg-primary-50">
        <div className="container-custom max-w-4xl">
          ...
          <div className="card border-l-4 border-accent-500">
            <p className="text-base sm:text-lg text-primary-700 leading-relaxed italic">
            ...
            <div className="card">
              ...
              <ul className="space-y-4">
                {LAWYER_PROFILE.certifications.map((cert, index) => (
                  <li key={index} className="flex items-start">
                    <div className="flex-shrink-0 w-2 h-2 mt-2 bg-accent-500 rounded-full mr-4"></div>
                    <span className="text-primary-700 text-sm sm:text-base">{cert}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            {LAWYER_PROFILE.specializations.map((spec, index) => (
              ...
              >
                <div className="flex items-center">
                  <span className="text-2xl sm:text-3xl mr-4 transform group-hover:scale-110 transition-transform duration-300">
                    ⚖️
                  </span>
                  <span className="text-base sm:text-lg font-medium text-primary-900">{spec}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="container-custom max-w-4xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {LAWYER_PROFILE.memberships.map((membership, index) => (
              <div 
                key={index} 
                className="card text-center bg-primary-50 hover:bg-accent-50 transition-colors duration-200"
              >
                <p className="text-primary-900 font-medium text-sm sm:text-base">{membership}</p>
              </div>
            ))}
          </div>
        </div>
